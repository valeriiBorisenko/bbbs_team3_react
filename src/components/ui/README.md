...
- **id** - number, id мероприятия, **required**
...
- **day** - string, день мероприятия.
- **month** - string, месяц мероприятия.
- **title** - string, название мероприятия.
...
- **isSelected** - boolean, если true, то кнопка является выделенной (как чекбокс), она становится полностью цветной, её текст меняется на titleSelected, если он передан. По умолчанию false.
...
- **handleUserButtonClick** - func, функция обработчик клика по svg-иконке пользователя.
