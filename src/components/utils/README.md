...
- **max** - string, максимальное числовое значение
- **min** - string,  минимальное числовое значение
- **minLength** - string, минимальное числовое значение знаков
- **maxLength** - string,  максимальное числовое значение знаков
...
- **isNested** - boolean, флаг того, что лоадер располагается в сетке (у него появляется верхний и нижний отступ).
...
Компонент-контейнер с горизонтальным скроллом (визуально скролл скрыт) - прокрутка колёсиком мыши, а также захватом мышкой. Используется в Личном кабинете (Profile).
...
Компонент используется в header'e. Представляет собой кликабельную кнопку с иконкой лупы. В данный момент открывает информационный попап.
